...
class Model():
	...
	def send_vendor_publication(self, data):
		#pub_opts = dbus.Dictionary(signature='sv')
		pub_opts = dbus.Dictionary({ "Vendor": dbus.UInt16(0xa000) }, signature='sv')

		print('Send vendor publication ', end='')
		print(data)
		node.Publish(self.path, self.model_id, pub_opts, data,
						reply_handler=generic_reply_cb,
						error_handler=generic_error_cb)
	...
	def send_message(self, dest, key, data):
		...
		for databyte in data:
			if type(databyte) == int:
				...
				print(('%02x ') % databyte, end='')	#python3
...
class OnOffClient(Model):
	...
	def set_publish(self, dest, key, state):
		opcode = 0x8203
		print('Set state:', state)
		self.data = struct.pack('>HBB', opcode, state, self.tid)
		self.tid = (self.tid + 1) % 255
		#self.data = struct.pack('>HB', opcode, state)
		#self.data = struct.pack('>B', state)
		self.send_publication(self.data)
...
class MainMenu(Menu):
	...
	def __send_vendor_msg(self, str_value):
		...
		if (dst_addr <= 0x7fff):
			app.elements[0].models[1].send_message(dst_addr, app_idx,msg_data)
		else:
		 	app.elements[0].models[1].send_vendor_publication(msg_data)
...
class ClientMenu(Menu):
	...
	def __cmd_set_state_off(self):
		if (dst_addr <= 0x7fff):
			...
			app.elements[1].models[0].set_publish(dst_addr, app_idx, 0)

	def __cmd_set_state_on(self):
		if (dst_addr <= 0x7fff):
			app.elements[1].models[0].set_state(dst_addr, app_idx, 1)
		else:
			app.elements[1].models[0].set_publish(dst_addr, app_idx, 1)
...
def vendor_send_callback(message):
	...
	print("message from bluetooth_mesh/send is", message)
	if message.msg.data[0] == '\x82':
		if message.msg.data[1] == '\x01':
			...
			if (dst_addr <= 0x7fff):	
				...
				app.elements[1].models[0].set_publish(message.msg.dest, app_idx, data)

	#vendor
	#Cx xx xx
	else:
		if (dst_addr <= 0x7fff):
			app.elements[0].models[1].send_message(message.msg.dest, app_idx, message.msg.data)
		else:
			app.elements[0].models[1].send_vendor_publication(message.msg.data)

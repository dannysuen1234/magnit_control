...
class MainMenu(Menu):
	...
	def __cmd_join(self):
		...
		#uuid_bytes = '\x0a\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f'
		uuid_bytes = uuid.uuid4().bytes
		#uuid_bytes = bytearray.fromhex("0a0102030405060708090A0B0C0D0E0F")
		#random.shuffle(uuid_bytes)
		#print(type(uuid_bytes))
		uuid_str = array_to_string(uuid_bytes)
		#uuid_bytearray = uuid_bytes.encode('utf8')
		uuid_bytearray = uuid_bytes
		#print(uuid_bytearray)

		print(set_yellow('Joining with UUID ') + set_green(uuid_str))
		#mesh_net.Join(app.get_path(), uuid_bytes,
		mesh_net.Join(app.get_path(), uuid_bytearray,
